...
from .constants import COMET_ABI, ERC20_ABI, PRICE_FEED_ABI
...
def get_collateral_balance(wallet: EvmWalletProvider, compound_address: str, asset_address: str) -> int:
    ...
    """
    return wallet.read_contract(compound_address, COMET_ABI, "collateralBalanceOf", args=[wallet.get_address(), asset_address])


def get_borrow_details(wallet: EvmWalletProvider, compound_address: str) -> dict[str, float]:
    ...
    """
    borrow_amount_raw = wallet.read_contract(compound_address, COMET_ABI, "borrowBalanceOf", args=[wallet.get_address()])
    ...
    human_borrow_amount = float(format_amount_from_decimals(borrow_amount_raw, base_decimals))
    price = float(Decimal(base_price_raw) / Decimal(10**8))

    return {"Token Symbol": base_token_symbol, "Borrow Amount": human_borrow_amount, "Price": price}


def get_supply_details(wallet: EvmWalletProvider, compound_address: str) -> list[dict[str, float]]:
    ...
    for i in range(num_assets):
        ...
        if collateral_balance > 0:
            ...
            human_supply_amount = float(format_amount_from_decimals(collateral_balance, decimals))
            price = float(Decimal(price_raw) / Decimal(10**8))
            collateral_factor = float(Decimal(asset_info[4]) / Decimal(10**18))
...
def get_health_ratio(wallet: EvmWalletProvider, compound_address: str) -> float:
    ...
    health_ratio = float("inf") if borrow_value == 0 else float(total_adjusted_collateral / borrow_value)
    return health_ratio


def get_health_ratio_after_borrow(wallet: EvmWalletProvider, compound_address: str, borrow_amount: str) -> float:
    ...
    return (
        ...
        else float(total_adjusted_collateral / new_borrow_value)
    )
...
def get_health_ratio_after_withdraw(
    ...
    decimals = get_token_decimals(wallet, asset_address)
    withdraw_human = Decimal(format_amount_from_decimals(int(withdraw_amount), decimals))
...
def get_portfolio_details_markdown(wallet: EvmWalletProvider, comet_address: str) -> str:
    ...
    if supply_details:
        for supply in supply_details:
            ...
            asset_value = float(supply_amount) * price
    ...
    if borrow_amount > 0:
        ...
        borrow_value = float(borrow_amount) * price
