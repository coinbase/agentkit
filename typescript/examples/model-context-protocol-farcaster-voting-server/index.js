...
const server = new McpServer({
  ...
  version: "1.0.0",
  description: "MCP Server for voting on BaseBookvsMovie Farcaster Mini App on Base blockchain",
});
...
server.tool(
  ...
  "Build a vote transaction for BaseBookvsMovie. Returns the transaction data to be signed and sent by the wallet.",
  {
    walletAddress: z.string().describe("The voter's wallet address (0x...)"),
  ...
  async ({ walletAddress, count }) => {
    ...
    const transactions = selected.map(titleId => {
    ...
    const summary = transactions
      .map(t => `- ${t.title}: ${t.vote}`)
      .join("\n");

    return {
      content: [{
        type: "text",
        text: `Prepared ${actualCount} vote transactions:\n\n${summary}\n\nTransaction data ready. Please sign and send each transaction using your wallet.`,
      }],
      data: { transactions }
    };
  }
