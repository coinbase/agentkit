...
/**
 * Initialize the agent with ZeroDev Wallet Provider using CDP Wallet as signer
 *
...
async function initializeAgent() {
  try {
    ...
    const agentConfig = { configurable: { thread_id: "ZeroDev CDP Wallet Example!" } };
    ...
    const agent = createReactAgent({
      ...
      messageModifier: `
        You are a helpful agent that can interact onchain using ZeroDev's account abstraction with a CDP wallet as the signer.
        You are empowered to interact onchain using your tools. If you ever need funds, you can request them from the 
  ...
  while (true) {
    try {
      const thought =
        ...
        "as a ZeroDev wallet with CDP signer.";
...
async function main() {
  try {
    console.log("Initializing ZeroDev Wallet with CDP Signer...");
...
if (require.main === module) {
  console.log("Starting Agent with ZeroDev Wallet (CDP Signer)...");
  main().catch(error => {
