...
export class CdpSmartWalletProvider extends EvmWalletProvider implements WalletProviderWithClient {
  ...
  public static async configureWithWallet(
    config: CdpWalletProviderConfig & { smartAccountName?: string } = {},
    ...
    const ownerAccount = await (config.address
    ...
    const smartAccount = await (config.smartAccountName
      ? cdpClient.evm.getOrCreateSmartAccount({
  ...
  async exportWallet(): Promise<{ name: string | undefined; address: `0x${string}` }> {
    return {
      ...
      address: this.#smartAccount.address as `0x${string}`,
    };
  }
